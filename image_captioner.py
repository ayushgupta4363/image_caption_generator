...
Original file is located at
    https://colab.research.google.com/drive/1E1T7rrPq5nHuVnVPbSsjDJ8z7GAHX8A_
...
# Make a directory for Kaggle credentials
!mkdir -p ~/.kaggle

# Upload your kaggle.json file. In Colab, you can do this by clicking the folder icon on the left panel, then 'Upload to session storage'.
# Or, if you've already stored it as a Colab secret, uncomment the following line and set the secret name to 'KAGGLE_TOKEN'.
# from google.colab import userdata
# import json
# with open('/root/.kaggle/kaggle.json', 'w') as f:
#     json.dump(json.loads(userdata.get('KAGGLE_TOKEN')), f)

# Ensure kaggle.json has correct permissions
!chmod 600 ~/.kaggle/kaggle.json

# Download the dataset (replace 'flickr8k' with the actual dataset slug if different)
!kaggle datasets download -d adityajn105/flickr8k

# Unzip the dataset into the INPUT_DIR
!unzip -q flickr8k.zip -d {INPUT_DIR}

# Verify the contents of the INPUT_DIR after unzipping
print(f"Contents of {INPUT_DIR}:")
!ls -F {INPUT_DIR}

# List contents of the /content directory to locate the unzipped files
print('Contents of /content:')
!ls -F /content

"""Please examine the output above. If you see another `flickr8k/` directory inside `INPUT_DIR`, then the actual path to your images might be `/content/flickr8k/flickr8k/Images`. You would then need to adjust the `INPUT_DIR` or move the contents."""
...
The previous `INPUT_DIR` was set for a Kaggle environment. To run this notebook in Google Colab, you need to ensure the `flickr8k` dataset is accessible.

You can typically do this by:

1.  **Uploading the dataset:** Manually upload the `flickr8k.zip` file to your Colab environment (e.g., `/content/`).
2.  **Mounting Google Drive:** If your dataset is in Google Drive, mount your drive and update the `INPUT_DIR` to point to its location.
3.  **Downloading directly:** Use shell commands (e.g., `!wget` or `!gdown`) to download the dataset to your Colab environment, then unzip it.

Once you have the `flickr8k` folder (containing `Images` and `captions.txt`) in your Colab environment, please update the `INPUT_DIR` variable below to its correct path. For demonstration, I will assume it's unzipped into `/content/flickr8k`.
"""

# Setting the input and output directory for Colab
        ...
        for image_id in data_keys:
            ...
            for caption in captions:
                ...
                for i in range(1, len(caption_seq)):
                    ...
                    out_seq = tf.keras.utils.to_categorical([out_seq], num_classes=vocab_size)[0] # Use tf.keras.utils.to_categorical
                    ...
                    if batch_count == batch_size:
                        # Yield a tuple where the first element is a tuple of tensors for inputs
                        yield (tf.convert_to_tensor(np.array(X1_batch)), tf.convert_to_tensor(np.array(X2_batch))), tf.convert_to_tensor(np.array(y_batch))
                        X1_batch, X2_batch, y_batch = [], [], []
                        batch_count = 0
...
# The generator yields: ([image_features_tensor, in_seq_tensor], out_seq_tensor)
# Note: The first element of the tuple is now a tuple of TensorSpecs, not a list.
output_signature = (
    (tf.TensorSpec(shape=(None, 4096), dtype=tf.float32),  # Image features
     tf.TensorSpec(shape=(None, max_caption_length), dtype=tf.int32)),  # Input sequence
    tf.TensorSpec(shape=(None, vocab_size), dtype=tf.float32)  # Output sequence (one-hot encoded)
)
...
for epoch in range(epochs):
    ...
    # Create tf.data.Dataset from generators with explicit output_signature
    train_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(train, image_to_captions_mapping, loaded_features, tokenizer, max_caption_length, vocab_size, batch_size),
        output_signature=output_signature
    )
    test_dataset = tf.data.Dataset.from_generator(
        lambda: data_generator(test, image_to_captions_mapping, loaded_features, tokenizer, max_caption_length, vocab_size, batch_size),
        output_signature=output_signature
    )
...
"""After running the above cells, the `flickr8k` dataset should be in `/content/flickr8k`, and you should find the `Images` directory and `captions.txt` within it. Then, you can re-run the cell to extract image features."""
